...
class FIBSEMDataset:
    ...
    e.g., "Crop 10"
    dimensions: A `dict` with keys specifying axes and values specifying the dimension, in pixels, of the dataset along each axis, 
    e.g., {'z':10, 'y':10, 'x':10} for a 10x10x10 pixel dataset
    offset: A `dict` with keys specifying axes and values specifying the relative offset of the origin, in pixels, of the dataset along each axis.
    e.g., {'z':0, 'y':0, 'x':0} for a dataset that starts in the corner of a parent dataset.
    resolution: A `dict` with keys specifying axes and values specifying the resolution of the dataset along each axis,
    e.g., {'z':8, 'y':4, 'x':4}
    labels: A list of two lists describing the labeled fields identified in the dataset. The first list contains the integer codes 
    for the labels, and the second list contains the string names for the coded labels. 
